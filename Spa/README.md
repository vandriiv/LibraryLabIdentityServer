...
Instead of downloading the entire app before User can use it, code splitting allows you to split your code into small chunks which you can then load on demand.
...
* Missing files will not be called at compilation time, and will cause 404 errors for your User.
 ...
 * Your app will work regardless of network state, even if offline. This means your User will be able to use your app at 10,000 feet and on the subway.
 * On mobile devices, your app can be added directly to the user's home screen, app icon and all. You can also re-engage User using web **push notifications**. This eliminates the need for the app store.
...
have decided that you would like to disable them for all your existing User,
...
When User install your app to the homescreen of their device the default configuration will make a shortcut to `/index.html`. This may not work for client-side routers which expect the app to be served from `/`. Edit the web app manifest at [`public/manifest.json`](public/manifest.json) and change `start_url` to match the required URL scheme, for example:
